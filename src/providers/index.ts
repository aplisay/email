...
export const SUPPORTED_PROVIDERS = ["smtp2go", "console"] as const;

/** Resolve a provider from config (driven by EMAIL_SEND_TYPE). */
export function createProvider(config: EmailClientConfig): EmailProvider {
  ...
  switch (type) {
    ...
    default:
      throw new EmailError(
        `Unknown EMAIL_SEND_TYPE "${type}". Supported: ${SUPPORTED_PROVIDERS.join(", ")}.`,
      );
  }
}
